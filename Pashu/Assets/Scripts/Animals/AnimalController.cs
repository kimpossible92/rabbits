...
public class AnimalController : MonoBehaviour
{
    ...
    {
        target = t;
        currentState = State.ToPoint;
    }
    public void SetTheBomb(Vector3 t)
    {
        target = t;
        currentState = State.SetTheBomb;
    }
    ...
    void Update()
    {
        ...
        if (currentState == State.SetTheBomb)
        {
            if (Vector3.Distance(transform.position, target) <= 2f)
            {
                var p = Instantiate(Bomb, target, Quaternion.identity);
                FindObjectOfType<cameraController>().setBoolBomb(false);
            }
        }
        if (currentState == State.ToPoint)
        {
    ...
    public void OnMouseDown()
    {
        ...
        cameraController.instance.SetAnimal(this);
    }
}
